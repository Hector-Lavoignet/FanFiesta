...
class AdaptadorChat(contexto: Context, chatLista : List<Chat>, imagenUrl : String):RecyclerView.Adapter<AdaptadorChat.ViewHolder?>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            imagen_enviada_izquierdo = itemView.findViewById(R.id.imagen_enviado_izquierdo)
            TXT_mensaje_visto = itemView.findViewById(R.id.TXT_mensaje_visto)
            imagen_enviada_derecha = itemView.findViewById(R.id.imagen_enviado_derecha)
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return if (position == 1) {
            val view: View = LayoutInflater.from(contexto).inflate(
                me.hectorlavoignet.proyectorubenborrador.R.layout.item_mensaje_derecho,
                parent,
                false
            )
            ViewHolder(view)
            ...
            val view: View = LayoutInflater.from(contexto).inflate(
                me.hectorlavoignet.proyectorubenborrador.R.layout.item_mensaje_izquierdo,
                parent,
                false
            )
            ViewHolder(view)
